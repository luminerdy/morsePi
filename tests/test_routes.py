...
class RouteRenderTests(unittest.TestCase):
    ...
    def test_touch_warmup_shows_letter_with_morse_review(self):
    ...
    def test_signal_drop_frontend_has_bounded_adaptive_speed_and_pause(self):
        ...
        self.assertIn('mode === "send" && values.reason !== "bottom"', source)
        self.assertIn("markTargetMiss(target)", source)
        self.assertIn('mode === "send" ? "MISS" : "Not yet"', source)
        self.assertIn(".signal-drop-layout", css)
        self.assertIn("background: #c62828", css)
