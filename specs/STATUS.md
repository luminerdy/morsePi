...
| Truthful hardened updater and IoT verification | `28934a8`...`4aa22f5` + live Pappy verification 2026-08-29 | FR-035/FR-058/API-027/AC-031/TEST-019 — update lock, preserved dirty patch, nonzero blocked/rollback results, commit verification, self-refreshing update services, sanitized cloud/Admin status, and read-only remote diagnostics implemented after Astrid/Liara exposed a false-success dirty-checkout skip; live rollback and expected-commit IoT success are proven |
...
| AWS IoT Jobs remote update foundation | `22a7589` + `f806498` + Campbell live verification 2026-08-29 | FR-058/API-027/SEC-022/AC-031/TEST-019 — local polling worker, systemd service/timer, station config examples, least-privilege IoT Jobs policy template, docs, and regression tests are implemented. AWS Things and per-station inline Jobs data-plane policies are provisioned for all three station IDs. Pappy and Campbell/Olivea now run the hardened release with local and IoT update timers active; Astrid/Liara consumed an earlier live Job but still needs its one-time dirty-checkout recovery. |
...
| Private family station activity | main `fae38c6`...`f6a2072`; release/pi `2151b93`...`fe6349d` | FR-064/DR-023/API-029/SEC-023/AC-040/TEST-025 — confirmed updates, progress uploads, and message milestones publish as privacy-limited duplicate-safe events with offline retry. Pappy has a PIN-gated 800x480 activity dashboard and is the sole member of an exact-prefix reader group. All 285 Pi tests pass; live Pappy refresh reads all three status records with zero errors. |
| Bounded touch admin session | main `7135fa7`; release/pi `b543895` | FR-065/API-030/SEC-024/AC-041/TEST-026 — one PIN unlock authorizes all touch admin areas for a sliding 10-minute session; Exit Admin, student navigation, timeout, and app restart relock it. The opaque cookie contains no PIN. All 290 Pi tests and locked/unlocked 800x480 rehearsals pass; Pappy is live. |
...
| FR-035 hardened updater | ✅ | `release/pi` branch, serialized requests, dirty-tree patch preservation, truthful nonzero blocks, fast-forward only, pre-restart tests, ending-commit verification, health check, explicit rollback reports, and status/snapshot refresh |
...
| FR-058/API-027/SEC-022 remote update Jobs | 🟡 | Local worker, timer, AWS Things/policies, truthful local-report verification, optional expected-commit check, and read-only diagnostics are implemented. Pappy consumed both already-current and real fast-forward Jobs. Campbell updated locally from `49ce373` to `e7f996b`, passed all 261 Pi tests, installed browser supervision, enabled its IoT timer, and completed an AWS poll with `no-pending-job`; Astrid/Liara still requires one local dirty-checkout recovery. |
...
| FR-064 family station activity | ✅ | Durable privacy-limited event publishing, offline retry, Pappy-only PIN-gated history, station check-ins, filters, partial-cache preservation, and 30-minute refresh are implemented and live-rehearsed |
...
| API-029 family activity touch view | ✅ | Protected by the bounded touch admin session and refreshed from the privacy-limited Pappy cache |
| API-030 touch admin session | ✅ | Unlock uses an approved local next-route allowlist; Exit Admin revokes the server token; protected actions fail closed after expiry |
...
| AC-036 idle Morse screensaver | — | ✅ | The current 260-test suite and live Pappy checks pass: cyan Morse-only recall for 5 seconds, delayed yellow answer reveal for 3 seconds, no answer pixels at new-cycle boundaries, reserved stable answer space, random movement, and physical-keyer/touch wake verification. Pappy is deployed through application release `179ae62`. |
...
| AC-038 Signal Drop | — | ✅ | Route/selector tests, an 800x480 browser rehearsal, and live Pappy testing cover known-letter boundaries, weighted recent-letter selection, server validation, bonus-only logging, Send/Read layout, touch and physical-key answers, adaptive bounds, pause, corrective feedback, persistent red retry targets, and zero page scrolling. Pappy is deployed beyond the final Signal Drop correction through application release `179ae62`. |
| AC-040 family station activity | — | ✅ | Unit/integration tests cover deterministic replay, offline queue recovery, privacy allowlists, partial cache preservation, publisher checkpoints, and IAM scope. Live Pappy verification at `fe6349d` passes 285 tests, renders cleanly at 800x480, and refreshes three station statuses with zero cloud errors. |
| AC-041 bounded touch admin session | — | ✅ | Unlock, cookie flags, no-repeat actions, sliding timeout, explicit exit, student-flow relock, safe next targets, and restart invalidation are covered; Pappy passes 290 tests and both entry/unlocked screens fit 800x480 without scrolling. |
