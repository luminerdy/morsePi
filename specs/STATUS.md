...
| Private family station activity | main `fae38c6`...`1702d37`; release/pi `2151b93`...`3fe47db` | FR-064/DR-023/API-029/SEC-023/AC-040/TEST-025 — confirmed updates, progress uploads, and message milestones publish as privacy-limited duplicate-safe events with offline retry. Pappy's PIN-gated 800x480 dashboard renders its cache immediately and performs one visible nonblocking cloud refresh. All 292 Pi tests pass. |
...
| FR-064 family station activity | ✅ | Durable privacy-limited publishing, offline retry, Pappy-only history, station check-ins, filters, partial-cache preservation, immediate cached display, and single-flight background refresh are implemented |
...
| API-029 family activity touch view | ✅ | Protected by the bounded touch admin session; renders cache immediately and exposes only non-renewing refresh state while one cloud refresh runs |
...
| AC-040 family station activity | — | ✅ | Tests cover replay, offline recovery, privacy allowlists, partial cache preservation, publisher checkpoints, immediate cached rendering, single-flight refresh, and status auth. Pappy at `3fe47db` passes 292 tests. |
