...
| Student progress sync design, attempt IDs, dry-run report, upload-only, and manual full-sync path | 2026-08-04 working tree | FR-054, DR-019 — new Practice/Words/Sprint attempts include stable `attempt_id`; merge contract documented; dry-run report lists local upload candidates/conflicts/cloud errors without writing student files; manual upload-only and full `--sync` modes are available; automatic idle/timer sync remains future work |
...
| FR-054/DR-019 student progress sync | 🟡 | Merge strategy documented, new attempts get `attempt_id`, and `scripts/student_attempt_sync.py` produces a no-write dry-run report, upload-only path, and manual full sync with backup/merge/rebuild; automatic idle/timer sync remains future work |
