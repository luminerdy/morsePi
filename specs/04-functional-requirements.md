...
- **FR-059** Warm-Up Review SHALL be offered from Daily when a student has
  ...
  Practice menu SHALL also provide a manual Warm-Up entry point so a student or
  adult can start review on demand. The warm-up SHALL review only currently
...
- **FR-061** *(V1)* The touch application SHALL show a full-screen idle
  ...
  alone on a black background for 5 seconds, then reveal its matching
  character for 3 seconds while keeping the Morse visible. The character
...
- **FR-062** *(V1)* Deployed Pi stations SHALL run the Chromium kiosk as a
  ...
  run Chromium as its tracked main process, and restart it automatically after
  an unexpected exit. Installation SHALL be idempotent, remove legacy Labwc
  and XDG browser autostart entries only after the supervised service starts,
  and be applied by the normal station update path. The adult `Exit Kiosk`
  action SHALL stop supervision before closing Chromium so the recovery
