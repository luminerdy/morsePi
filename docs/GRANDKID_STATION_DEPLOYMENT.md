...
```text
s3://morsepi-backups-luminerdy/stations/<station-id>/backups/<timestamp>-<station-id>-pre-deploy.zip
...
- Confirm the student can:
  ...
  - use Clear
  - get back to Daily
  - open Power, cancel once, then understand to wait for the screen to go dark
    before turning off the CanaKit USB-C PiSwitch when they really shut down
